...
_EOT_
  ...
  declare -a _values=($(echo "${_response:-}" | python -c "import sys;import json;data=json.load(sys.stdin);[[print(value) for value in records['value']] for records in data['properties']['TXTRecords']] if 'properties' in data else False;"))
  _response=$(cat <<_EOT_|bash
